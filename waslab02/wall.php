...
switch ($_SERVER['REQUEST_METHOD']) {
	...
	case 'PUT':
		...
		$result = new SimpleXMLElement($dades);
		
		$id = $dbhandler->insertTweet($result->author,$result->text);
		echo $id;
		$postdata = new SimpleXMLElement("<response></response>");
		$postdata->addChild('tweetid' , $id);
		$postdata->tweetid->addAttribute('author', $result->author);
		echo $postdata->asXML();
        exit;
	...
	case 'DELETE':
		...
		$tweetid = $_GET['twid'];
		echo $tweetid;
		$succes = $dbhandler->deleteTweet($tweetid);
		$postdata = new SimpleXMLElement("<response></response>");
		$postdata->addChild('deletion' , $succes);
		$postdata->deletion->addAttribute('tweetid', $tweetid);
		echo $postdata->asXML();
		exit;
